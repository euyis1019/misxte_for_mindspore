...
def project_to_2d(X, camera_params):
    ...
    XX = mindspore.ops.clamp(X[..., :2] / X[..., 2:], min=-1, max=1)
    r2 = mindspore.ops.sum(XX[..., :2]**2, dim=len(XX.shape)-1, keepdim=True)

    #注意下面的mindspore.ops.cat不支持精度转化，如果有低精度Tensor需要手动提升精度
    radial = 1 + mindspore.ops.sum(k * mindspore.ops.cat((r2, r2**2, r2**3), axis=len(r2.shape)-1), dim=len(r2.shape)-1, keepdim=True)
    tan = mindspore.ops.sum(p*XX, dim=len(XX.shape)-1, keepdim=True)
...
def uvd2xyz(uvd, gt_3D, cam):
    ...
    #检查xy和z_global能否在axis=-1的情况下链接，axis=-1的情况就是说要保证除了最后一个维度以外的其他维度全部一致才行
    xyz_global = mindspore.ops.cat((xy, z_global), -1)  # N*T*V*3
