...
def mpjpe(predicted, target, return_joints_err=False):
    ...
    if not return_joints_err:
        return mindspore.ops.mean(mindspore.ops.norm(predicted - target, dim=len(target.shape)-1))
    else:
        errors = mindspore.ops.norm(predicted - target, dim=len(target.shape)-1)
        # errors: [B, T, N]
        from einops import rearrange
        errors = rearrange(errors, 'B T N -> N (B T)')
        errors = mindspore.ops.mean(errors, axis=-1).cpu().numpy().reshape(-1) * 1000
        return mindspore.ops.mean(mindspore.ops.norm(predicted - target, dim=len(target.shape)-1)), errors
...
def weighted_mpjpe(predicted, target, w):
    ...
    return mindspore.ops.mean(w * mindspore.ops.norm(predicted - target, dim=len(target.shape)-1))
...
def n_mpjpe(predicted, target):
    ...
    norm_predicted = mindspore.ops.mean(mindspore.ops.sum(predicted**2, dim=3, keepdim=True), axis=2, keep_dims=True)
    norm_target = mindspore.ops.mean(mindspore.ops.sum(target*predicted, dim=3, keepdim=True), axis=2, keep_dims=True)
...
def mean_velocity_error_train(predicted, target, axis=0):
    ...
    velocity_predicted = mindspore.ops.diff(predicted, axis=axis)
    velocity_target = mindspore.ops.diff(target, axis=axis)

    return mindspore.ops.mean(mindspore.ops.norm(velocity_predicted - velocity_target, dim=len(target.shape)-1))
...
def sym_penalty(dataset,keypoints,pred_out):
    ...
    if dataset == 'h36m':
        ...
        for (i_left,j_left),(i_right,j_right) in zip(left_bone,right_bone):
            ...
            loss_sym += mindspore.ops.mean(mindspore.ops.abs(mindspore.ops.norm(left_part, dim=-1) - mindspore.ops.norm(right_part, dim=-1)))
...
def bonelen_consistency_loss(dataset,keypoints,pred_out):
    ...
    if dataset == 'h36m':
        ...
        for (i,j) in bones:
            ...
            loss_length += mindspore.ops.mean(mindspore.ops.norm(bone_diff, dim=-1))
